...
\input{generated/anisotropic_acoustic_matrix.tex}
...
\input{generated/isotropic_acoustic_matrix.tex}
...
Given $E(x)$, $S(x)$ we want to compute
\begin{equation}
\frac{\delta J}{\delta \overline{\varepsilon}(x)},\qquad\frac{\delta J}{\delta C(x)}\label{eqn:func_deriv_anisotropic}
\end{equation}
...
\end{equation}
with $\tau(x) = \left(C(x)-C^0(x)\right)\varepsilon(x)$. This can be written in condensed form as $\varepsilon = \overline{\varepsilon}-\Gamma^0 * \left((C-C^0)\varepsilon\right)$ Define the residual operator
\begin{equation}
...
\end{equation}
The Lippmann-Schwinger iteration for solving the adjoint equation in \eqref{eqn:adjoint_equation} is shown in \cref{alg:lippmann_schwinger_adjoint}. Alternatively one can use the fact that $C^0\Gamma^0*\Lambda=\Lambda$ to write the method in the incremental form shown in \cref{alg:lippmann_schwinger_adjoint_incremental}.
\begin{algorithm}
...
\end{algorithm}
\begin{algorithm}
    \caption{Adjoint Lippmann Schwinger iteration (incremental form)}
    \label{alg:lippmann_schwinger_adjoint_incremental}
\begin{algorithmic}[1]
    \State{Set $\Lambda=0$}
    \For{$n=0,1,\dots$}
    \State{Compute $\widehat{\Lambda}_{ij} = \mathscr{F}[\Lambda_{ij}]$}\Comment{Fourier-transform}
    \State{Set $\widehat{\Theta}_{k\ell} = -\widehat{\Gamma}^0_{k\ell ij} \widehat{\Lambda}_{ij}$}\Comment{Solve in Fourier space}
    \State{Compute $\Theta_{k\ell} = \mathscr{F}^{-1}[\widehat{\Theta}_{k\ell}]$}\Comment{Inverse Fourier-transform}        
    \State{Compute $\Delta_{ij}=C_{ijk\ell}\Theta_{k\ell}$}
    \State{Update $\Lambda_{ij}\mapsto\Lambda_{ij}+\mathcal{E}_{ij}+\Delta_{ij}$}
    \State{Check convergence}
    \EndFor
\State{\Return $\varepsilon$}
\end{algorithmic}
\end{algorithm}
Comparing \cref{alg:lippmann_schwinger_incremental} \cref{alg:lippmann_schwinger_adjoint_incremental}, we see that the primal- and adjoint Lippmann Schwinger iteration only differ in the order in which $\Gamma^0$ and $C$ are applied and in the update step. Also, the algorithm is readily applied to both the isotropic and anisotropic case by use the appropriate way of computing $\Delta = C\Theta$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{equation}
    ...
    &= \left(\Gamma^0_{ijab}(x-z)\Lambda_{ab}(z)\;dx\right)\varepsilon_{k\ell}(x)\\
    &= \left(\Gamma^0*\Lambda\right)_{ij}(x)\varepsilon_{k\ell}(x)
    \end{aligned}
\end{equation}
Hence, putting everything together, we get
\begin{equation}
\frac{\delta \mathcal{L}}{\delta C_{ijk\ell}} = \left(S+\Gamma^0*\Lambda \right)_{ij}\varepsilon_{k\ell}
\end{equation}
...
\begin{equation}
\begin{aligned}
\frac{\delta \mathcal{L}}{\delta \lambda} &= \operatorname{tr}\left(S+\Gamma^0*\Lambda \right)\operatorname{tr}(\varepsilon),\\
\frac{\delta \mathcal{L}}{\delta \mu} &= \left(S+\Gamma^0*\Lambda \right):\varepsilon.\\
\end{aligned}
\end{equation}
