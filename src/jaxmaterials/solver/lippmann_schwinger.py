...
@contextmanager
def redirect_stdout():
    """Context manager for redirecting the output of CUDA library to ``sys.stdout``

    Since the CUDA library uses ``printf()``, its output is not written to Python's
    ``sys.stdout``. This context manager redirects the output generated by the ctypes
    library to a temporary file and then prints the output using a Python ``print()``
    statement which can be captured.
    """
    stdout_id = 1  # id of stdout
    # Backup standard output fd
    saved_fd = os.dup(stdout_id)

    with tempfile.TemporaryFile(mode="w+b") as tmp_file:
        # Redirect stdout to temporary file
        os.dup2(tmp_file.fileno(), stdout_id)

        try:
            yield
        finally:
            # Flush standard out (capture any Python print statements in the same block)
            sys.stdout.flush()
            # Flush ctypes output
            ctypes.CDLL(None).fflush(None)
            # Restore sydout to the backed up value
            os.dup2(saved_fd, stdout_id)
            os.close(saved_fd)
            # Print content of temporary file, so that it can subsequently be captured
            # when inspecting ``sys.stdout``
            tmp_file.seek(0)
        ...
        with redirect_stdout():
            its = cuda_code(
                ...
                np.ascontiguousarray(
                    jnp.broadcast_to(
                        _delta_epsilon_initial, shape=(6, *grid_spec.extents)
                    ),
                    dtype=np.float32,
                ),
                epsilon_cuda,
