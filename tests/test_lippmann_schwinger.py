...
def test_fourier_solve(grid_spec, dtype):
    ...
    r_hat_anisotropic = fourier_solve_anisotropic(sigma_hat, N_ref, xizero)[:, 0, 0, 1]

    rtol = 1.0e-7 if dtype == np.float32 else 1.0e-12
    print(np.linalg.norm(r_hat_isotropic))
    print(np.linalg.norm(r_hat_anisotropic))
    print(np.linalg.norm(r_hat_anisotropic - r_hat_isotropic))
    assert (
